...
class TestLlamaInference:
    ...
    @patch("elpis.llm.backends.llama_cpp.inference.Llama")
    @patch("elpis.llm.backends.llama_cpp.inference.detect_hardware")
    ...
    def test_model_load_failure(self, mock_llama):
        ...
        config = LlamaCppConfig(path="/nonexistent/model.gguf", hardware_backend="cpu")

        with pytest.raises(ModelLoadError):
            LlamaInference(config)
    ...
    async def test_chat_completion_success(self, mock_llama):
        ...
        config = LlamaCppConfig(path="/test/model.gguf", hardware_backend="cpu")
        llm = LlamaInference(config)
    ...
    def test_capability_flags(self):
        ...
        assert LlamaInference.SUPPORTS_STEERING is False
        assert LlamaInference.MODULATION_TYPE == "sampling"
