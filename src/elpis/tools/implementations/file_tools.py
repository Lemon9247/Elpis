...
from elpis.utils.exceptions import PathSafetyError, ToolExecutionError
...
class FileTools:
    ...
    def _read_file_sync(self, file_path: str, max_lines: int) -> Dict[str, Any]:
        ...
        try:
            ...
            if file_size > self.settings.tools.max_file_size:
                return {
                    'success': False,
                    'error': f"File too large: {file_size} bytes (max: {self.settings.tools.max_file_size})"
                }
